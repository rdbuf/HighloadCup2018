// http://quick-bench.com/jZ27fJei1efL_Z-2jeh9a4O4eeA
...
static void Set(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    for (int i = 0; i < N / 10; ++i) z.insert(rand());
    auto intersection = s; s.intersect(z);
  }
}
...
static void StdSet(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    std::set_intersection(s.begin(), s.end(), z.begin(), z.end(), std::inserter(intersection, intersection.end()));
  }
}
BENCHMARK(StdSet);
